...
struct X_TYPED_LIST : public X_LIST_ENTRY {
 public:
  ...
  template <typename VirtualTranslator>
  TObject* UnlinkHeadObject(VirtualTranslator vt) {
    uint32_t old = util::XeRemoveHeadList(this, vt);

    return ListEntryObject(vt->TranslateVirtual<X_LIST_ENTRY*>(old));
  }

  template <typename VirtualTranslator>
  TObject* UnlinkTailObject(VirtualTranslator vt) {
    uint32_t old = util::XeRemoveTailList(this, vt);
